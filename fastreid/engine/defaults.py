...
from fastreid.data import build_reid_train_loader_forcenter, build_reid_test_loader, build_reid_train_loader, build_reid_gallery_loader, build_reid_query_loader
from fastreid.evaluation import (ReidEvaluator,
                                 inference_on_dataset, print_csv_format, inference_on_bct_dataset)
...
class DefaultTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        self.scheduler = self.build_lr_scheduler(
            cfg, optimizer, self.iters_per_epoch)
    ...
    @classmethod
    def build_train_loader_center(cls, cfg):
        """
        Returns:
            iterable
        It now calls :func:`fastreid.data.build_detection_train_loader`.
        ...
        logger = logging.getLogger(__name__)
        logger.info("Prepare training set")
        return build_reid_train_loader_forcenter(cfg)
    @classmethod
    ...
    @classmethod
    def testcenter(cls, cfg,model,train_data):
        sums={}
        nums={}
        center={}
        model.eval()
        ...
        for data in train_data:
            #print(data['targets'])
            target=data['targets'].cpu().detach().numpy()
            idx=idx+1
            # False use bnfeat (特征有正有负)
            # True use pool5 (特征为正)
            bn_feat=model(data,forcenter=False).cpu().detach().numpy()
            for i in range(len(target)):
                if target[i] not in nums:
                    nums[target[i]]=1
                    sums[target[i]]=bn_feat[i]
                else :
                    sums[target[i]]=sums[target[i]]+bn_feat[i]
                    nums[target[i]]=nums[target[i]]+1
                if target[i] == 0:
                    c = sums[target[i]]/nums[target[i]]

        for i in sums:
            center[i]=sums[i]/nums[i]
    ...
    @classmethod
    def testcenterac(cls, cfg,model,train_data):
        sums={}
        nums={}
        center={}
        model.eval()
        ...
        try:
            ...
            print(e)
        #CENTERPATH = "./logs_final/market1501/bagtricks_R18-softmax-part/all-center-bn-save.pkl"
        #center=pickle.load(open(CENTERPATH,'rb'))
        ...
        #     minidx=-1
        #     for j in range(lens):
        #             dis = np.linalg.norm(res[i]-center[j])
        #             if mindis>dis:
        #                 mindis=dis
        #                 minidx=j 
        #     print(i, minidx)
        ...
        #print(center[:20])
        #print(res[:20])
        #mixinds=np.array([ 0 for i in range(lens)])
        #mixinds[:lens//2]=1
        #np.random.shuffle(mixinds)
        #tmpid = np.reshape(mixinds, ( len(mixinds),1))
        #tmpcenter = tmpid*center + (1-tmpid)*res
        ...
        for data in train_data:
            ...
            for i in range(len(target)):
                ...
                minidx=-1
                for j in range(lens):
                    dis = np.linalg.norm(bn_feat[i]-res[j])
                    if mindis>dis:
                        mindis=dis
                        minidx=j 
                if minidx == target[i]:
                    ...
                    #    print(mindis)
                    #    ccc = ccc + 1
        print("the peobability that a feature belongs to the class of its nearest center is {}".format(ac/tot))
        return res 

    @classmethod
    def test(cls, cfg, model):
        if cfg.OLDMODEL.CONFIG == "":
            print('common test.....')
            return cls.test_ori(cfg, model)
        else:
            cls.test_ori(cfg, model)
            print('cross test......')
            return cls.test_for_compatibility(cfg, model)

    @classmethod
    def test_ori(cls, cfg, model):
        """
        Args:
            cfg (CfgNode):
            model (nn.Module):
    ...
    @classmethod
    def test_for_compatibility(cls, cfg, model):
        """
        Args:
            cfg (CfgNode):
            model (nn.Module):
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TESTS):
            logger.info("Prepare testing set")
            try:
                query_loader, num_query = build_reid_query_loader(cfg, dataset_name)
                gallery_loader = build_reid_gallery_loader(cfg, dataset_name)
                evaluator = ReidEvaluator(cfg, num_query=num_query)
            except NotImplementedError:
                logger.warn(
                    "No evaluator found. implement its `build_evaluator` method."
                )
                results[dataset_name] = {}
                continue
            results_i = inference_on_bct_dataset(
                model, query_loader, gallery_loader, evaluator, flip_test=cfg.TEST.FLIP_ENABLED,rate=cfg.TEST.OLDRATE)
            results[dataset_name] = results_i
        ...
        if comm.is_main_process():
            ...
            print_csv_format(results, eval_type='bce')
...
for _attr in ["model", "data_loader", "optimizer"]:
    setattr(DefaultTrainer, _attr, property(
